...
FIX S21 (APR 1, 2026):
  - BUG-SR-1: Applied H5-equivalent fix to create_strategy_from_breakout_detector()
    which still had silent bare except. Now logs warning on unexpected exceptions.
  - BUG-SR-2: create_custom_strategy() error log upgraded info -> warning.
"""
...
def create_strategy_from_breakout_detector(lookback_bars: int = 12,
    ...
    def strategy(bars: List[Dict], params: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            logger.warning(
                f"[REPLAY] BreakoutDetector unavailable (ImportError: {e}). "
                "Signals will be None for all bars in this run."
            )
            return None

        except Exception as e:
            logger.warning(f"[REPLAY] BreakoutDetector error on bar: {e}")
            return None
...
def create_custom_strategy(signal_logic: Callable) -> Callable:
    ...
    def strategy(bars: List[Dict], params: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            logger.warning(f"[BACKTEST] Strategy error: {e}")
            return None
...
if __name__ == "__main__":
    ...
    logger.info("=" * 80)
    logger.info("Use create_strategy_from_breakout_detector() to wrap your actual signal logic.")
