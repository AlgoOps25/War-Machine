...
class SignalGenerator:
    ...
    def check_ticker(self, ticker: str, use_5m: bool = True) -> Optional[Dict]:
        ...
        if self.validator and bars:
            try:
                ...
                conf_emoji = "\U0001f4c8" if conf_change > 0 else "\U0001f4c9" if conf_change < 0 else "\u27a1\ufe0f"
        ...
        if ML_BOOSTER_ENABLED and self.ml_booster and self.ml_booster.is_trained:
            try:
                ...
                if abs(adjustment * 100) > 1.0:
                    emoji = "\U0001f4c8" if adjustment > 0 else "\U0001f4c9"
        ...
        if MTF_ENABLED and mtf_validator:
            try:
                ...
                if mtf_result['confidence_boost'] > 0:
                    ...
                    print(f"[MTF-BOOST] {ticker} \U0001f504 | "
        ...
        if UOA_ENABLED and uoa_detector:
            try:
                ...
                if whale_data['is_unusual'] and whale_data['confidence_boost'] > 0:
                    ...
                    print(f"[UOA-BOOST] {ticker} \U0001f40b | "
                          f"Conf: {original_conf:.0f}% \u2192 {boosted_conf:.0f}% "
        ...
        if OR_ENABLED and or_detector:
            try:
                ...
                if 'or_boost' in signal and signal['or_boost'] > 0:
                    ...
                    print(f"[OR-BOOST] {ticker} \U0001f3af | "
    ...
    def scan_watchlist(self, watchlist: List[str], use_5m: bool = True) -> List[Dict]:
        ...
        Market Hours Gate: Returns an empty list immediately if called outside
        regular trading hours (weekends, pre-market, or after-hours).
        ...
        """
        # \u2500\u2500\u2500 Market Hours Gate \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500
        # Prevents pre-market, after-hours, and weekend signals from ever
        # being generated, regardless of which scheduler calls this method.
        if not is_market_hours():
            _log_market_hours_block()
            return []
        # \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500
    ...
    def send_signal_alert(self, signal: Dict, send_discord: bool = True) -> None:
        ...
        # Console output
        print("\n" + "="*70)
        print(f"\U0001f6a8 BREAKOUT SIGNAL DETECTED: {ticker}")
        print("="*70)
    ...
    def _format_discord_alert(self, signal: Dict) -> str:
        ...
        # Build message
        msg = f"\U0001f6a8 **{ticker} {direction} BREAKOUT** \U0001f6a8\n\n"

        # Entry guidance
        entry_range_low = entry * 0.9985
        entry_range_high = entry * 1.0015
        msg += f"\U0001f3af **ENTRY GUIDANCE:**\n"
        ...
        if 'target_method' in signal and signal['target_method'] != 'fixed_rmultiples':
            ...
            msg += f"   \U0001f4ca Targets: {method_name} ({target_conf:.0%} confidence)\n"

        msg += "\n"

        # Signal quality
        msg += f"\U0001f4ca **SIGNAL QUALITY:**\n"
        ...
        if 'or' in signal:
            or_data = signal['or']
            or_emoji = "\U0001f3af" if or_data['classification'] == 'TIGHT' else "\u26a0\ufe0f" if or_data['classification'] == 'WIDE' else "\u2705"
        ...
        if 'options_dte' in signal:
            ...
            if selected_dte is not None:
                msg += f"\U0001f4c8 **OPTIONS RECOMMENDATION:**\n"
                ...
                # Skip signal
                msg += f"\U0001f6ab **OPTIONS:** {dte_info.get('reasoning', 'No recommendation available')}\n\n"
        else:
            # Fallback time-based
            now_et = datetime.now(ET)
            ...
            if hours_left >= 2.5:
                ...
                msg += f"\U0001f4c5 **OPTIONS RECOMMENDATION:**\n"
                msg += f"   \U0001f4c6 1DTE (Expires Tomorrow 4:00 PM)\n"
                msg += f"   Reason: Limited time today ({hours_left:.1f} hrs)\n\n"
            else:
                msg += f"\U0001f6ab **OPTIONS:** Too close to market close\n\n"
