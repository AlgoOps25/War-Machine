...
#
# BUG-SC-1 FIX (2026-03-31): Added blank line between `import logging` and
#   `logger = ...` per PEP 8 / codebase convention.
...
def _check_confidence_inversion(ticker: str, grade: str, rvol: Optional[float]) -> None:
    ...
    if grade == "A+" and rvol is not None and rvol < 1.2:
        logger.warning(
            f"[{ticker}] CONFIDENCE-INVERSION: A+ grade but RVOL={rvol:.2f}x < 1.2x — "
...
def build_scorecard(
    ...
    try:
        ...
        logger.warning(f"[SCORECARD] build_scorecard error — blocking signal as precaution: {e}")
