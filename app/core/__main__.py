...
BOOT ORDER (critical for Railway health-check):
  ...
  3. import scanner         — triggers module-level DB connect (~10s)
  4. start_scanner_loop()   — enter the main loop

Previously health server was started inside start_scanner_loop() which
meant the port was dark during the DB connect block, causing Railway to
...
# ── 2. Health server UP before the DB block fires on scanner import ───────────
#    (scanner.py has a module-level psycopg2.connect that blocks ~10s)
from app.core.health_server import start_health_server
start_health_server()

# ── 3. Now safe to import scanner (triggers module-level DB connect) ──────────
