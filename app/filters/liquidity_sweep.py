...
def _candle_swept_level(bar: dict, level: float, direction: str) -> bool:
    """
    ...
    if direction == "bull":
        ...
        close_reclaim = (bar["close"] - level) / level >= -SWEEP_CLOSE_MAX_PCT
...
def detect_liquidity_sweep(
    ...
    if direction == "bull" and or_low:
        levels.append((or_low, "OR_LOW"))
    if direction == "bear" and or_high:
        levels.append((or_high, "OR_HIGH"))
    if vwap and vwap > 0:
        levels.append((vwap, "VWAP"))

    for i, bar in enumerate(scan_bars):
        for level, label in levels:
            if _candle_swept_level(bar, level, direction):
                return {
                    "swept_level":    level,
