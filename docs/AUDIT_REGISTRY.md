...
| 8 | ~~🟢 LOW~~ | ~~`app/ml/ml_confidence_boost.py`~~ | ~~BUG-ML-5: `.iterrows()` in logging loop — replace with `itertuples()`~~ | ✅ Confirmed fixed — already `.itertuples()` (Queue #8 Apr 03 2026) |
...
| 16 | ~~🟢 LOW~~ | ~~`app/core/eod_reporter.py`~~ | ~~BUG-EOD-1: `win_rate` pulled from `daily_stats` — verify `get_session_status()` always returns 0–100 scale.~~ | ✅ Closed — full `win_rate` scale audit (Apr 03 2026). All 15 consumers verified. `get_daily_stats()` returns 0–100 (display only). All gating consumers (`dynamic_thresholds`, `entry_timing`, `hourly_gate`) compute own 0–1 ratio from raw DB counts. Zero mismatches. |
...
| 110 | 2026-04-02 | P4-2 | `app/validation/entry_timing.py` | 🔧 47.P4-2 COMPLETE: Real HOURLY_WIN_RATES patched from 5-ticker walk-forward (AAPL/AMD/MSFT/NVDA/TSLA, 55 trades, Apr 02 2026). Hour 10: (0.54, 26) — 54% WR, gating live. Hour 15: (0.67, 12) — 67% WR, golden hour, gating live. Hours 9/11–14: (0.50, 0) — insufficient data. MIN_SAMPLE_SIZE lowered 20→10 to align with script floor. Stale 4.C-10 comment replaced with 47.P4-2 provenance. | `dcf0478` | P4-2 ✅ COMPLETE |
