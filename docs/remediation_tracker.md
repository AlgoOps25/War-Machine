...
|--------|----|------|-------------|------------|------|
| ⬜ | 44.H-2 | `utils/time_helpers.py` | `_strip_tz()` drops TZ without converting to ET first — root cause of 15+ tz-naive bugs across 20+ files. Fix: `dt.astimezone(ET).replace(tzinfo=None)` | — | — |
