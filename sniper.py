...
def _bar_time(bar):
    #Safely extract time from a bar's datetime field.#
    bt = bar.get("datetime")
...
def log_proposed_trade(ticker, signal_type, direction, price, confidence, grade):
    #Logs a proposed trade for win-rate tracking of signals that weren't necessarily taken.#
    try:
...
def arm_ticker(ticker, direction, zone_low, zone_high, or_low, or_high, 
    ...
    # Log the proposed trade for tracking
...
def process_ticker(ticker: str):
    ...
    #1. Update memory DB via data_manager
    #2. Get bars from memory
    #3. Compute Opening Range (9:30-9:40 ET)
    #4. Detect ORB breakout
    #5. Detect FVG after breakout
    #6. CFW6 candle confirmation (A+/A/A-)
    #7. Multi-factor confirmation layers
    #8. Calculate stops & targets
    #9. Get options recommendation
    #10. Compute final confidence (AI + MTF boost)
    #11. ARM ticker & open position
    ...
    try:
        ...
        # STEP 3 — OPENING RANGE (9:30-9:40 ET)
        or_high, or_low = compute_opening_range_from_bars(bars)
        if or_high is None: return
        ...
        # wait_for_confirmation returns 5-tuple: (found, entry_price, grade, confirm_idx, confirm_type)
        found, entry_price, base_grade, confirm_idx, confirm_type = result
        if not found or base_grade == "reject": return
        
        # STEP 7 — MULTI-FACTOR CONFIRMATION LAYERS
        ...
        # STEP 10 — COMPUTE CONFIDENCE (AI learning + MTF convergence boost)
