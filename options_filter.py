...
INTEGRATED: IV Rank (IVR) computation from historical iv_history table.
"""
...
class OptionsFilter:
    ...
    def find_best_strike(self, ticker: str, direction: str,
        ...
        # ── IVR enrichment ────────────────────────────────────────────────
        # After the best strike is found, compute IV Rank from historical
        # observations and attach it to the options dict.  store_iv_observation()
        # persists the current IV so future calls to compute_ivr() have data.
        if best_option:
            ...
            if iv and iv > 0:
                ...
                best_option["ivr"]            = ivr
                best_option["ivr_obs"]         = obs
                best_option["ivr_reliable"]    = reliable
                best_option["ivr_multiplier"]  = mult
                best_option["ivr_label"]       = label

                status = f"IVR={ivr:.0f}" if (ivr is not None and reliable) else "IVR-BUILDING"
                print(f"[IVR] {ticker}: IV={iv*100:.1f}% | {status} "
                      f"({obs} obs) | Multiplier: {mult:.2f}x | [{label}]")
            else:
                # IV unavailable — attach neutral IVR fields so sniper never KeyErrors
                best_option["ivr"]           = None
                best_option["ivr_obs"]        = 0
                best_option["ivr_reliable"]   = False
                best_option["ivr_multiplier"] = 1.0
                best_option["ivr_label"]      = "IVR-NO-DATA"
    ...
    def validate_signal_for_options(self, ticker: str, direction: str,
        ...
        if price_move_needed > expected_move * 2:
            return False, best_strike, (
                f"Target requires ${price_move_needed:.2f} but "
                f"expected move only ${expected_move:.2f}"
            )
        ...
        if mid > 0 and dte > 0:
            ...
            if theta_pct > config.MAX_THETA_DECAY_PCT:
                return False, best_strike, (
                    f"Theta decay too high: {theta_pct*100:.2f}% per day"
                )
