...
NOTE: This script is for LOCAL development use only.
- Reads from SQLite marketmemory.db (not production PostgreSQL)
- Uses MLConfidenceBooster (ml_confidence_boost.py), not the newer ml_trainer.py
- Run manually when you want to retrain the confidence_booster.pkl model
  against locally-accumulated trade logs.

For production retraining against PostgreSQL signal_analytics, use:
  scripts/train_from_analytics.py  (or app/ml/train_from_analytics.py)
"""
