...
Usage: python scripts/audit_repo.py [--output-dir ./audit_reports]

NOTE: This is a standalone developer tool. It is NOT imported by any production
module and is NOT run during deployment. Safe to run locally or in CI.
...
# Configuration
REPO_ROOT = Path(__file__).parent.parent.absolute()  # scripts/ -> repo root
...
class CodeAuditor:
    ...
    def security_scan(self):
        ...
        report = ["Security Vulnerability Scan", "=" * 80, ""]
        ...
        try:
            ...
            if "No issues identified" not in result.stdout:
                self.issues.append({
                    ...
                    'file': 'Multiple', 'message': 'Bandit found security issues - review bandit output'
                })
        except FileNotFoundError:
    ...
    def dependency_audit(self):
        ...
        report = ["Dependency Security Audit", "=" * 80, ""]
    ...
    def analyze_architecture(self):
        ...
        report = ["Architecture Analysis", "=" * 80, ""]
    ...
    def analyze_documentation(self):
        ...
        report = ["Documentation Coverage Analysis", "=" * 80, ""]
    ...
    def code_quality_analysis(self):
        ...
        report = ["Code Quality Analysis (Pylint)", "=" * 80, ""]
        ...
        try:
            ...
            if "unused-import" in result.stdout:
                self.issues.append({
                    ...
                    'file': 'Multiple', 'message': 'Unused imports detected - run pylint for details'
                })
        except FileNotFoundError:
    ...
    def trading_system_audit(self):
        ...
        report = ["Trading System Health Audit", "=" * 80, ""]
        ...
        for name, path in critical_files.items():
            ...
            if not exists:
                self.issues.append({
                    'severity': 'CRITICAL', 'category': 'Trading System',
                    'file': path, 'message': f'Critical file missing: {name}'
                })

        model_path = self.repo_path / 'models' / 'signal_predictor.pkl'
        report.append(f"  ML Model: {'OK' if model_path.exists() else 'Not found (expected — in .gitignore)'}")
    ...
    def git_analysis(self):
        ...
        report = ["Git History Analysis", "=" * 80, ""]

        try:
            for cmd, title in [
                (["git", "log", "--oneline", "-20"], "RECENT COMMITS (Last 20)"),
    ...
    def generate_summary(self):
        ...
        report.append("-" * 80)
        for report_file in sorted(self.output_dir.glob("*.txt")):
