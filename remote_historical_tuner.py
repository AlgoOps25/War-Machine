...
class RemoteHistoricalTuner:
    ...
    def _get_historical_trades(self, min_trades: int = 20) -> List[Dict]:
        """Retrieve historical trades from Railway database."""
        conn = self._get_connection()
        cursor = conn.cursor(cursor_factory=RealDictCursor)
        
        try:
            # Query only columns we know exist
            cursor.execute("""
                SELECT 
                    ticker,
                    grade,
            ...
            if len(trades) < min_trades:
                ...
                if len(trades) < 10:
                    print(f"[WARNING] Too few trades for meaningful analysis. Continue trading to collect more data.\n")
                else:
    ...
    def analyze_stop_widths(self, trades: List[Dict]) -> Dict:
        ...
        for grade in ['A+', 'A', 'A-']:
            ...
            avg_width = statistics.mean(stop_widths)
    ...
    def generate_report(self) -> str:
        ...
        if len(trades) < 10:
            return (
                ...
                f"Found {len(trades)} trades. Need minimum 10 closed trades.\n"
        ...
        if len(trades) < 20:
            lines.append("⚠️  WARNING: Sample size is small. Recommendations may not be statistically significant.")
            lines.append("    Continue trading to collect more data for better analysis.\n")
        ...
        if not stop_analysis.get('by_grade'):
            lines.append("\nNo stop loss data available.\n")
        else:
            for grade in ['A+', 'A', 'A-']:
        ...
        if not ticker_analysis.get('recommended_whitelist') and not ticker_analysis.get('recommended_blacklist'):
            lines.append("\nNo strong ticker patterns identified yet (need 5+ trades per ticker with 70%+ or <40% win rate).")
        
        # Show all tickers with 3+ trades
        lines.append("\nAll Tickers (3+ trades):")
        sorted_tickers = sorted(
            ticker_analysis['by_ticker'].items(),
        ...
        lines.append("2. Review ticker performance for position selection preferences")
